...
var app = angular.module('syllabusApp', ['general.controllers', 'general.services', 'general.directives', 'general.filters', 'syllabusApp.controllers', 'syllabusApp.directives', 'ngRoute', 'ngSanitize']);
...
app.controller('syllabusCtrl', ['$scope', '$timeout', '$window', 'wordsSrv', 'CONFIG', function ($scope, $timeout, $window, wordsSrv, config) {
    $scope.words = null;
    $scope.total = 0;
    $scope.perPage = 10;
    $scope.currentPage = 0;
    ...
    $scope.getPageOfWords = function (currentPage) {
        $timeout.cancel($scope.timeoutPromise);
        wordsSrv.getPageOfWords(currentPage, $scope.perPage, $scope.sectionFilter, '').then(function (data) {
    ...
    $scope.addNewWord = function () {
        $timeout.cancel($scope.timeoutPromise);
        wordsSrv.postWord($scope.newWord, $scope.newSection).then(function (data) {
            $scope.messages.success = data.successMessage;
            $scope.currentPage = 0;
    ...
    $scope.editWord = function (word) {
        $timeout.cancel($scope.timeoutPromise);
        wordsSrv.putWord(word.id, word.word, word.section).then(function (data) {
            $scope.messages.success = data.successMessage;
        }, function (error) {
            $scope.messages.error = error.errorMessage;
        }).finally(function () {
    ...
    $scope.deleteWord = function (wordid) {
        ...
        $timeout(function () {
            ...
            wordsSrv.deleteWord(wordid).then(function () {
...
app.directive('wordListItemLong', ['$timeout', 'CONFIG', function ($timeout, config) {
    return {
        restrict: 'A',
        scope: {
            word: '=',
