...
var app = angular.module('cardsApp', ['general.controllers', 'general.services', 'general.directives', 'general.filters', 'cardsApp.controllers', 'cardsApp.directives', 'ngRoute', 'ngSanitize']);
...
app.controller('cardsListCtrl', ['$scope', '$timeout', 'initialRouteSrv', 'cardsSrv', 'messageSrv', 'CONFIG', function ($scope, $timeout, initialRouteSrv, cardsSrv, messageSrv, config) {
    $scope.perPage = 10;
    $scope.cards = null;
    $scope.total = 0;
    $scope.currentPage = 0;
    ...
    $scope.getPageOfStudentCardsInActivity = function (currentPage) {
        ...
        cardsSrv.getPageOfStudentCardsInActivity(currentPage, $scope.perPage, config.instanceid).then(function (data) {
...
app.controller('cardFeedbackListCtrl', ['$scope', '$timeout', 'cardsSrv', 'messageSrv', 'CONFIG', function ($scope, $timeout, cardsSrv, messageSrv, config) {
    $scope.perPage = 10;
    $scope.cards = null;
    $scope.total = 0;
    $scope.currentPage = 0;
    ...
    $scope.getPageOfCardsInReviewInActivity = function (currentPage) {
        $timeout.cancel($scope.timeoutPromise);
        cardsSrv.getPageOfCardsInReviewInActivity(currentPage, $scope.perPage, config.instanceid).then(function (data) {
            $scope.cards = data.cards;
            $scope.total = data.total;
        }, function (error) {
            $scope.cards = null;
...
app.directive('cardListItem', ['CONFIG', function (config) {
    return {
        restrict: 'A',
        scope: {
            card: '='
...
app.directive('cardFeedbackListItem', ['CONFIG', function (config) {
    return {
        restrict: 'A',
        scope: {
            card: '='
...
app.directive('cardMenu', ['$location', 'CONFIG', function ($location, config) {
    var titles = ['edit_cards', 'review_cards'];
    var routes = ['/card/list', '/card/feedback'];
    return {
        restrict: 'E',
        scope: {},
